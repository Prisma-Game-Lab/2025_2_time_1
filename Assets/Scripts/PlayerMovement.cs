...
public class PlayerMovement : MonoBehaviour, IDamageable
{
    ...
    private void ApplyAttack(float force, bool heavy, int damage)
    {
        ...
        // Lógica de arremessar objetos (Se estiver segurando algo)
        if (heldObject != null)
        {
        ...
        if (Physics.Raycast(attackRay, out RaycastHit hit, range))
        {
            ...
            //Verifica se acertou Inimigo ou Cenário
            ...
            {
                if (heavy)
                    effectToSpawn = heavyAttackEffect;
                else
            ...
            if (effectToSpawn != null)
            {
                ...
                VisualEffect vfxInstance = Instantiate(effectToSpawn, spawnPoint, Quaternion.LookRotation(hit.normal));
    ...
    private void PerformStrongPunch()
    {
        ...
        if (Physics.Raycast(attackRay, out RaycastHit hit, range))
        {
            ...
            if (hit.collider.CompareTag("Enemy") || hit.transform.root.CompareTag("Enemy"))
            {
                // Lógica Específica do Inimigo (EnemyAI)
            ...
            // Instanciação do Efeito
            if (effectToSpawn != null)
            {
                Vector3 spawnPoint = hit.point + (hit.normal * 0.1f);
                VisualEffect vfxInstance = Instantiate(effectToSpawn, spawnPoint, Quaternion.LookRotation(hit.normal));

                // Super soco sempre tem muito sangue
                if (vfxInstance.HasInt("BloodCount"))
                {
                    vfxInstance.SetInt("BloodCount", 50);
                }
